...
def get_package_version():
    with open(Path(this_dir) / "mamba_plus" / "__init__.py", "r") as f:
...
def get_wheel_url():
    ...
    mamba_plus_version = get_package_version()
    ...
    wheel_filename = f"{PACKAGE_NAME}-{mamba_plus_version}+cu{cuda_version}torch{torch_version}cxx11abi{cxx11_abi}-{python_version}-{python_version}-{platform_name}.whl"
    wheel_url = BASE_WHEEL_URL.format(
        tag_name=f"v{mamba_plus_version}", wheel_name=wheel_filename
    )
...
setup(
    ...
    author="Aobo Liang",
    author_email="liangaobo@bupt.edu.cn",
    description="Mamba plus",
