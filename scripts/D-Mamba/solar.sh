...
if [ "$dataset_name" = "weather" ]; then
    ...
    root_path="/mnt/data/lab/Bi-Mamba4TS/data/Solar"
    data_path="solar.txt"
    dataset_type=custom
    enc_in=137
fi
...
do
    ...
    do
        log_file="${dataset_name}(${random_seed})_${model}(${seq_len}-${pred_len}).log"
        nohup python $work_space $model \
        --gpu=$gpu \
        --embed_type=2 --num_workers=4 --seed=$random_seed --batch_size=$batch_size \
